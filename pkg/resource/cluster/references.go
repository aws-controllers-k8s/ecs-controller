...
func (rm *resourceManager) ClearResolvedReferences(res acktypes.AWSResource) acktypes.AWSResource {
	...
	if len(ko.Spec.CapacityProviderRefs) > 0 {
...
func (rm *resourceManager) ResolveReferences(
	...
	if fieldHasReferences, err := rm.resolveReferenceForCapacityProviders(ctx, apiReader, ko); err != nil {
...
func validateReferenceFields(ko *svcapitypes.Cluster) error {
	...
	if len(ko.Spec.CapacityProviderRefs) > 0 && len(ko.Spec.CapacityProviders) > 0 {
		return ackerr.ResourceReferenceAndIDNotSupportedFor("CapacityProviders", "CapacityProviderRefs")
	}
	return nil
}

// resolveReferenceForCapacityProviders reads the resource referenced
// from CapacityProviderRefs field and sets the CapacityProviders
// from referenced resource. Returns a boolean indicating whether a reference
// contains references, or an error
func (rm *resourceManager) resolveReferenceForCapacityProviders(
	ctx context.Context,
	apiReader client.Reader,
	ko *svcapitypes.Cluster,
) (hasReferences bool, err error) {
	for _, f0iter := range ko.Spec.CapacityProviderRefs {
		if f0iter != nil && f0iter.From != nil {
			hasReferences = true
			arr := f0iter.From
			if arr.Name == nil || *arr.Name == "" {
				return hasReferences, fmt.Errorf("provided resource reference is nil or empty: CapacityProviderRefs")
			}
			...
			obj := &svcapitypes.CapacityProvider{}
			if err := getReferencedResourceState_CapacityProvider(ctx, apiReader, obj, *arr.Name, namespace); err != nil {
				return hasReferences, err
			}
			if ko.Spec.CapacityProviders == nil {
				ko.Spec.CapacityProviders = make([]*string, 0, 1)
			}
			ko.Spec.CapacityProviders = append(ko.Spec.CapacityProviders, (*string)(obj.Spec.Name))
		}
	}
