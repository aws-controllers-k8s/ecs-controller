...
func (rm *resourceManager) EnsureReferences(
	...
	if desiredKO.Spec.NetworkConfiguration != nil {
		if desiredKO.Spec.NetworkConfiguration.AWSVPCConfiguration != nil {
			if len(desiredKO.Spec.NetworkConfiguration.AWSVPCConfiguration.SecurityGroupRefs) > 0 {
				if latestKO.Spec.NetworkConfiguration == nil {
					latestKO.Spec.NetworkConfiguration = &svcapitypes.NetworkConfiguration{}
				}
				if latestKO.Spec.NetworkConfiguration.AWSVPCConfiguration == nil {
					latestKO.Spec.NetworkConfiguration.AWSVPCConfiguration = &svcapitypes.AWSVPCConfiguration{}
				}
				if len(latestKO.Spec.NetworkConfiguration.AWSVPCConfiguration.SecurityGroupRefs) == 0 {
					latestKO.Spec.NetworkConfiguration.AWSVPCConfiguration.SecurityGroupRefs = desiredKO.Spec.NetworkConfiguration.AWSVPCConfiguration.SecurityGroupRefs
				}
			}
			if len(desiredKO.Spec.NetworkConfiguration.AWSVPCConfiguration.SubnetRefs) > 0 {
				if latestKO.Spec.NetworkConfiguration == nil {
					latestKO.Spec.NetworkConfiguration = &svcapitypes.NetworkConfiguration{}
				}
				if latestKO.Spec.NetworkConfiguration.AWSVPCConfiguration == nil {
				...
				if len(latestKO.Spec.NetworkConfiguration.AWSVPCConfiguration.SubnetRefs) == 0 {
					latestKO.Spec.NetworkConfiguration.AWSVPCConfiguration.SubnetRefs = desiredKO.Spec.NetworkConfiguration.AWSVPCConfiguration.SubnetRefs
				}
			}
		}
	}
